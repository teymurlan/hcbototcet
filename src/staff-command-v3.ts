...
(function(){
  ...
  async function openNotifications(){pushOrigin();notifyShell();try{await loadNotify(true);renderNotifyTab('feed')}catch(e){var p=document.getElementById('hcV3NotifyPane');if(p)p.innerHTML='<div class="hc-v3-empty">'+esc(e.message)+'</div>'}}
