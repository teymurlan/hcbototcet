...
<script>
(function(){
  var accessState=null,patching=false;
  ...
  function beautifyVisits(){
    document.querySelectorAll('.rt-visit').forEach(function(row){
      if(row.dataset.hcVisitCard)return;
      var date=row.querySelector('.rt-date'),time=row.querySelector('.rt-time'),num=row.querySelector('b');
      if(!date||!time||!num)return;
  ...
  function patchRole(){var role=document.getElementById('role');if(!role||!accessState||!accessState.admin)return;role.textContent=accessState.owner?'Руководитель':'Администратор'}
  function moreScreenVisible(){return [].slice.call(document.querySelectorAll('.section-title h2')).some(function(x){return String(x.textContent||'').trim()==='Ещё'})}
  function patchAdminCard(){
    ...
    main.appendChild(card);document.getElementById('hcAdminAccessOpen').onclick=openAdminAccess;
  }
  ...
  async function openAdminAccess(){
    try{
      ...
      document.body.appendChild(modal);document.getElementById('hcAccessClose').onclick=function(){modal.remove()};modal.onclick=function(e){if(e.target===modal)modal.remove()};
  ...
  var mo=new MutationObserver(function(){requestAnimationFrame(patch)});mo.observe(document.documentElement,{subtree:true,childList:true});
  setInterval(patch,900);setTimeout(bootAccess,40);patch();
})();
</script>`;
