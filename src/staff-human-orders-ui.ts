...
(function(){
  ...
  function patch(){patchTimer=0;var main=document.getElementById('main');if(!main)return;replaceIds(main);prettyCards(main);attention(main);notices(main);statusBadges(main);patchSearch(main);main.querySelectorAll('.team-state.no').forEach(function(x){if(/нет команды/i.test(x.textContent||''))x.textContent='Нет назначенных сотрудников'});replaceIds(main)}
