...
interface TarotCard { position: number; card_name: string | null; orientation: "upright" | "reversed" | "unknown"; confidence: "high" | "medium" | "low"; }
interface TarotResult { readable: boolean; deck: string; cards: TarotCard[]; spread_type: string; analysis: string; conclusion: string; advice: string; }
...
async function handleUpdate(update: TelegramUpdate, env: Env): Promise<void> {
  ...
  const token = env.TELEGRAM_BOT_TOKEN; const chatId = message.chat.id;
  if (message.text === "/start") { await sendMessage(token, chatId, "🔮 Добро пожаловать.\n\nОтправьте одним сообщением фотографию расклада Таро и напишите вопрос в подписи к фотографии.\n\nЯ сначала определю видимые карты, их положение и порядок, а затем дам общий разбор именно по вашему вопросу. Если карта не читается уверенно, я не буду её придумывать."); return; }
  if (!message.photo?.length) { await sendMessage(token, chatId, "Пришлите фотографию расклада Таро и напишите вопрос в подписи к фотографии."); return; }
  const question = (message.caption || "").trim();
  if (!question) { await sendMessage(token, chatId, "Фото получил. Добавьте ваш вопрос в подпись к фотографии и отправьте фото ещё раз."); return; }
...
async function downloadTelegramImage(token: string, fileId: string): Promise<ArrayBuffer> {
  ...
  if (!imageBuffer.byteLength) throw new Error("Downloaded image is empty");
  return imageBuffer;
}
...
function normalizeTarotResult(value: any): TarotResult {
  ...
  return { readable: Boolean(value?.readable ?? cards.some(c => c.card_name)), deck: typeof value?.deck === "string" ? value.deck : "Не определена", cards, spread_type: typeof value?.spread_type === "string" ? value.spread_type : "Схема не определена", analysis: typeof value?.analysis === "string" ? value.analysis : "Не удалось сформировать анализ.", conclusion: typeof value?.conclusion === "string" ? value.conclusion : "Не удалось сформировать итог.", advice: typeof value?.advice === "string" ? value.advice : "Не удалось сформировать совет." };
}

function formatTarotResult(result: TarotResult, question: string): string {
  if (!result.cards.length || !result.cards.some(c => c.card_name)) return "Я не смог уверенно распознать карты на фотографии. Не хочу угадывать.\n\nПришлите фото крупнее, чтобы все карты были хорошо видны, без бликов и сильных теней.";
  const cardLines = result.cards.map(card => `${card.position}. ${card.card_name || "Карта не определена"}${card.orientation === "reversed" ? " — перевёрнутая" : card.orientation === "upright" ? " — прямая" : " — положение не определено"} (${card.confidence === "high" ? "высокая уверенность" : card.confidence === "medium" ? "средняя уверенность" : "низкая уверенность"})`).join("\n");
...
async function sendMessage(token: string, chatId: number, text: string): Promise<void> {
  const response = await fetch(`${TELEGRAM_API(token)}/sendMessage`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ chat_id: chatId, text, disable_web_page_preview: true }) });
