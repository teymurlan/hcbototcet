...
export default {
  async fetch(req: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (req.method === 'GET' && url.pathname === '/__hc_clients_health') return clientHealth(env);
    if (req.method === 'GET' && url.pathname === '/__hc_staff_catalog_design') {
      return healthJson({
        ...
        useful_dashboard_v3: true,
        unified_order_cards: true,
        origin_aware_back: true,
        stable_orders_entry: true,
...
async function clientHealth(env: Env): Promise<Response> {
  try {
    ...
    let subscriptions = 0, needsReview = 0;
    ...
    return healthJson({ok:true,clients:clients.length,subscriptions,needs_review:needsReview,imported_clients:Number(x?.meta?.clients||0),imported_visits:Number(x?.meta?.visits||0),imported_subscriptions:Number(x?.meta?.subscriptions||0)});
  } catch { return healthJson({ ok: false, error: 'client database unavailable' }, 503); }
}
