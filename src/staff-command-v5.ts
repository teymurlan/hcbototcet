...
html.hc-ui-density-compact #main>.card,html.hc-ui-density-compact #main>.order-card,html.hc-ui-density-compact #main>.task-card{padding-top:10px!important;padding-bottom:10px!important;margin-bottom:6px!important}html.hc-ui-density-compact .section-title{margin-top:14px!important;margin-bottom:7px!important}
html.hc-ui-motion-gentle #main.hc-motion-ready .metric,html.hc-ui-motion-gentle #main.hc-motion-ready .order-card,html.hc-ui-motion-gentle #main.hc-motion-ready .task-card,html.hc-ui-motion-gentle #main.hc-motion-ready .employee,html.hc-ui-motion-gentle #main.hc-motion-ready .notice-card{animation:none!important}html.hc-ui-motion-gentle #main.hc-motion-ready>*{animation-duration:.14s!important}
html.hc-ui-motion-off #main.hc-motion-ready *,html.hc-ui-motion-off .hc-nav-indicator,html.hc-ui-motion-off .hc-toast{animation:none!important;transition:none!important}
...
(function(){
 ...
 function normalizePrefs(v){v=v||{};return{style:['balanced','compact','focus'].indexOf(v.style)>=0?v.style:'balanced',density:['comfortable','compact'].indexOf(v.density)>=0?v.density:'comfortable',motion:['gentle','full','off'].indexOf(v.motion)>=0?v.motion:'gentle'}}
 ...
 function applyPrefs(v){v=normalizePrefs(v);dashboard=Object.assign({},dashboard||{},v);applyDashboard(v.style);var h=document.documentElement;h.classList.remove('hc-ui-density-comfortable','hc-ui-density-compact','hc-ui-motion-gentle','hc-ui-motion-full','hc-ui-motion-off');h.classList.add('hc-ui-density-'+v.density,'hc-ui-motion-'+v.motion)}
 async function loadDashboard(){if(dashboard){applyPrefs(dashboard);return dashboard}try{var x=await api('/api/staff/dashboard-preferences');dashboard=normalizePrefs(x.preferences)}catch(e){dashboard=normalizePrefs({})}applyPrefs(dashboard);return dashboard}
 async function decorateOverview(main){
   ...
   var b=document.createElement('button');b.className='hc-v5-view-button';b.type='button';b.dataset.v5View='1';b.innerHTML='<svg viewBox="0 0 24 24"><path d="M4 7h10M18 7h2M4 12h3M11 12h9M4 17h7M15 17h5"/><circle cx="16" cy="7" r="2"/><circle cx="9" cy="12" r="2"/><circle cx="13" cy="17" r="2"/></svg>Настроить';head.appendChild(b);
 }
 function openDashboardSheet(){
   ...
   var mask=document.createElement('div');mask.id='hcV5DashSheet';mask.className='hc-v5-sheet-mask';mask.innerHTML='<div class="hc-v5-sheet"><div class="hc-v5-sheet-head"><strong>Настроить приложение</strong><button data-v5-dash-close>×</button></div><div class="hc-v5-pref-group"><strong>Дизайн главной страницы</strong><div class="hc-v5-designs"><button class="hc-v5-design" data-v5-dash-style="balanced"><strong>Карточки</strong><p>Крупные действия и показатели.</p><span class="hc-v5-mini"><i></i><i></i><i></i><i></i></span></button><button class="hc-v5-design" data-v5-dash-style="compact"><strong>Компактный</strong><p>Больше данных на одном экране.</p><span class="hc-v5-mini"><i></i><i></i><i></i><i></i></span></button><button class="hc-v5-design" data-v5-dash-style="focus"><strong>Рабочий</strong><p>Заявки и оповещения крупнее.</p><span class="hc-v5-mini"><i></i><i></i><i></i><i></i></span></button></div></div><div class="hc-v5-pref-group"><strong>Плотность интерфейса</strong><div class="hc-v5-pref-row two"><button data-v5-pref="density" data-v5-value="comfortable">Удобная</button><button data-v5-pref="density" data-v5-value="compact">Компактная</button></div></div><div class="hc-v5-pref-group"><strong>Анимации</strong><div class="hc-v5-pref-row"><button data-v5-pref="motion" data-v5-value="gentle">Плавные</button><button data-v5-pref="motion" data-v5-value="full">Полные</button><button data-v5-pref="motion" data-v5-value="off">Выключить</button></div></div><div class="hc-v5-sheet-actions"><button class="cancel" data-v5-dash-close>Отмена</button><button class="save" data-v5-dash-save>Сохранить</button></div></div>';document.body.appendChild(mask);paintPrefs();
 }
 ...
 function queue(){if(queued)return;queued=true;requestAnimationFrame(function(){queued=false;var main=document.getElementById('main');if(!main)return;var p=currentPage();if(p==='overview')decorateOverview(main);if(p==='orders')decorateOrders(main);if(p==='notifications')upgradeNotifications();if(activeTab()==='more')decorateMore(main)})}
