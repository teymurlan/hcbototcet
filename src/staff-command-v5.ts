...
(function(){
 ...
 var queued=false,notifyTab='feed',noticeFilter='all',noticeQuery='',notices=null,settings=null,templates=null,broadcasts=null,employees=null,dashboard=null,dashboardBefore='balanced',ordersPending=false;
 ...
 var textRows=[['new_order','Новая заявка','Автоматическое сообщение руководителю'],['order_changed','Изменение заказа','Когда изменились данные заказа'],['order_cancelled','Отмена заказа','Когда заказ отменён'],['finance_changes','Реквизиты сотрудника','Когда сотрудник меняет реквизиты'],['broadcast','Рассылка','Шаблон ручных сообщений команде']];
 var sample={order:'Заказ #30',date:'18.09.2026',time:'14:30',address:'Санкт-Петербург, Невский проспект, 10',service:'Генеральная уборка',area:'65 м²',employee:'Алексей',title:'Изменение графика',body:'Завтра начинаем на час позже.',brand:'HOUSE CLEANING STAFF'};
 ...
 function renderTexts(pane){
   var x=templates||{},vals=x.templates||{},defs=x.defaults||{},ph=x.placeholders||{};
   pane.innerHTML='<div class="hc-v3-note"><b>Тексты сообщений</b><br>Редактируйте шаблоны, нажимайте переменные для вставки и проверяйте результат в предпросмотре. «Тест» отправит сообщение вам в Telegram.</div>'+textRows.map(function(r){var vars=(ph[r[0]]||[]).map(function(v){return'<button data-v5-var="'+esc(v)+'" data-v5-key="'+r[0]+'">{'+esc(v)+'}</button>'}).join('');return'<div class="hc-v5-template-card" data-v5-template="'+r[0]+'"><div class="hc-v5-template-top"><strong>'+esc(r[1])+'</strong><div class="hc-v5-template-actions"><button data-v5-reset="'+r[0]+'">Сбросить</button><button data-v5-test="'+r[0]+'">Тест</button></div></div><p>'+esc(r[2])+'</p><textarea data-v5-text="'+r[0]+'">'+esc(vals[r[0]]||'')+'</textarea><div class="hc-v5-vars">'+vars+'</div><div class="hc-v5-preview" data-v5-preview="'+r[0]+'"></div><button class="hc-v5-template-save" data-v5-save="'+r[0]+'">Сохранить текст</button></div>'}).join('');
   textRows.forEach(function(r){renderTemplatePreview(r[0])});
 }
