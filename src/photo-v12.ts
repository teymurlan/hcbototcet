...
export default {
  async fetch(req:Request,env:Env):Promise<Response>{
    ...
    let update:any;
    try{update=await req.clone().json()}catch{return photoV11.fetch(req,env)}

    const cb=update?.callback_query;
    if(cb?.from?.id){
      await handleCallback(cb,env,url.origin).catch(e=>console.error('v12 callback',e));
      return new Response('OK');
    }

    const m=update?.message;
    if(!m?.chat?.id||!m?.from?.id)return photoV11.fetch(req,env);
    const user:TgUser=m.from;
    const text=String(m.text||'').trim();
    const command=text.replace(/@\w+$/,'').toLowerCase();

    if(['/start','/menu','/webapp'].includes(command)||text==='🏠 Главное меню'){
      await sendStartV12(env,m.chat.id,user,url.origin);
      return new Response('OK');
    }
    if(command==='/rules'||text==='📋 Правила'){
      await sendRulesV12(env,m.chat.id,user,url.origin);
      return new Response('OK');
    }
    if(command==='/admin'||text==='👥 Сотрудники'){
      if(isAdmin(env,user.id))await sendAdminV12(env,m.chat.id);
      else await send(env.TELEGRAM_BOT_TOKEN,m.chat.id,'🔒 Управление сотрудниками доступно только администратору.');
      return new Response('OK');
    }
    if(text==='🧹 Открыть фотоотчёт'||text==='📷 Продолжить: ПОСЛЕ'){
      await sendFreshOpenButton(env,m.chat.id,user,url.origin,text.includes('ПОСЛЕ'));
      return new Response('OK');
    }
...
async function sendStartV12(env:Env,chatId:string|number,user:TgUser,origin:string){
  ...
  const status=!job||job.stage==='done'?'Нет активной уборки':job.stage==='started'?'Нужно отправить фото/видео ДО':'Фото ДО принято — уборка в процессе';
  const employeeName=access.employee?.name||displayName(user);
  const openText=job?.stage==='before_sent'?'🧹 Открыть текущую уборку':'🧹 Открыть фотоотчёт';

  const keyboard:any=[
    [{text:openText,web_app:{url:appUrl}}],
    [{text:'📋 Правила',callback_data:'v12:rules'}]
  ];
  if(access.admin)keyboard.push([{text:'👥 Сотрудники',callback_data:'v12:admin'}]);
...
async function sendFreshOpenButton(env:Env,chatId:string|number,user:TgUser,origin:string,forceAfter:boolean){
  const access=await getAccess(env,user.id);
  if(!access.allowed)return send(env.TELEGRAM_BOT_TOKEN,chatId,`🔒 Доступ закрыт. Ваш Telegram ID: <code>${user.id}</code>`,{parse_mode:'HTML'});
  const job=await getJob(env,user.id);
  const launch=await signLaunch(user,env.TELEGRAM_BOT_TOKEN);
  let appUrl=`${origin}/?launch=${encodeURIComponent(launch)}`;
  if(forceAfter&&job?.stage==='before_sent')appUrl+='&after=1';
  return send(env.TELEGRAM_BOT_TOKEN,chatId,'🔐 Создана новая персональная кнопка входа:',{reply_markup:{inline_keyboard:[[{text:forceAfter?'📷 Перейти к фото ПОСЛЕ':'🧹 Открыть приложение',web_app:{url:appUrl}}]]}});
}

async function sendRulesV12(env:Env,chatId:string|number,user:TgUser,origin:string){
  const access=await getAccess(env,user.id);
  const rows:any[]=[];
  if(access.allowed){
    const launch=await signLaunch(user,env.TELEGRAM_BOT_TOKEN);
    rows.push([{text:'🧹 Открыть фотоотчёт',web_app:{url:`${origin}/?launch=${encodeURIComponent(launch)}`}}]);
  }
  return send(env.TELEGRAM_BOT_TOKEN,chatId,`📋 <b>ПРАВИЛА ФОТООТЧЁТА</b>\n\n1️⃣ ДО отправляется до основной уборки.\n2️⃣ Все дефекты и повреждения фиксируются вместе с ДО.\n3️⃣ После уборки обязательно отправляется ПОСЛЕ.\n4️⃣ Время, сотрудник и GPS фиксируются автоматически.\n5️⃣ Незавершённая уборка сохраняется и восстанавливается.\n\n⚠️ Первый пропуск обязательного фотоотчёта — штраф, повторный — расторжение договора.`,{parse_mode:'HTML',...(rows.length?{reply_markup:{inline_keyboard:rows}}:{})});
}

async function handleCallback(cb:any,env:Env,origin:string){
  const id=Number(cb.from.id),chatId=cb.message?.chat?.id||id,data=String(cb.data||'');
  await tg(env.TELEGRAM_BOT_TOKEN,'answerCallbackQuery',{callback_query_id:cb.id}).catch(()=>null);
  if(data==='v12:rules')return sendRulesV12(env,chatId,cb.from,origin);
  if(data==='v12:admin'&&isAdmin(env,id))return sendAdminV12(env,chatId);
  if(!isAdmin(env,id))return;
  if(data==='v12:employees')return sendEmployeesV12(env,chatId);
  if(data==='v12:add'){await setPending(env,id,'add');return send(env.TELEGRAM_BOT_TOKEN,chatId,'➕ Отправьте одним сообщением:\n<code>TELEGRAM_ID Имя Фамилия</code>\n\nНапример: <code>123456789 Анна Иванова</code>',{parse_mode:'HTML'});}
  if(data==='v12:block'){await setPending(env,id,'block');return send(env.TELEGRAM_BOT_TOKEN,chatId,'🚫 Отправьте Telegram ID сотрудника для блокировки.');}
  if(data==='v12:unblock'){await setPending(env,id,'unblock');return send(env.TELEGRAM_BOT_TOKEN,chatId,'✅ Отправьте Telegram ID сотрудника для разблокировки.');}
  if(data==='v12:remove'){await setPending(env,id,'remove');return send(env.TELEGRAM_BOT_TOKEN,chatId,'🗑 Отправьте Telegram ID сотрудника для удаления.');}
}

async function sendAdminV12(env:Env,chatId:string|number){
  return send(env.TELEGRAM_BOT_TOKEN,chatId,'👑 <b>УПРАВЛЕНИЕ СОТРУДНИКАМИ</b>\n\nВыберите действие:',{parse_mode:'HTML',reply_markup:{inline_keyboard:[
    [{text:'➕ Добавить сотрудника',callback_data:'v12:add'}],
    [{text:'📋 Список сотрудников',callback_data:'v12:employees'}],
    [{text:'🚫 Заблокировать',callback_data:'v12:block'},{text:'✅ Разблокировать',callback_data:'v12:unblock'}],
    [{text:'🗑 Удалить сотрудника',callback_data:'v12:remove'}]
  ]}});
}

async function sendEmployeesV12(env:Env,chatId:string|number){
  const r=await stateCall(env,'/employees');const a:Employee[]=r.employees||[];
  if(!a.length)return send(env.TELEGRAM_BOT_TOKEN,chatId,'Список сотрудников пуст.');
  return send(env.TELEGRAM_BOT_TOKEN,chatId,'👥 <b>СОТРУДНИКИ</b>\n\n'+a.map((x,i)=>`${i+1}. ${x.status==='active'?'🟢':'🔴'} <b>${esc(x.name)}</b> · <code>${x.id}</code>`).join('\n'),{parse_mode:'HTML'});
}

async function removeOldKeyboard(env:Env,chatId:string|number){
  try{
    const x=await send(env.TELEGRAM_BOT_TOKEN,chatId,'Обновляем рабочее меню…',{reply_markup:{remove_keyboard:true}});
    const mid=x?.result?.message_id;if(mid)await tg(env.TELEGRAM_BOT_TOKEN,'deleteMessage',{chat_id:chatId,message_id:mid}).catch(()=>null);
  }catch{}
}
