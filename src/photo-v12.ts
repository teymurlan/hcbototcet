...
export default {
  async fetch(req:Request,env:Env):Promise<Response>{
    ...
    if(cb?.from?.id){await handleCallback(cb,env,url.origin).catch(async e=>{console.error('v12 callback',e);try{await tg(env.TELEGRAM_BOT_TOKEN,'answerCallbackQuery',{callback_query_id:cb.id,text:'Ошибка. Нажмите кнопку ещё раз.',show_alert:true})}catch{}});return new Response('OK');}
...
async function sendStartV12(env:Env,chatId:string|number,user:TgUser,origin:string){
  ...
  const keyboard:any=[[{text:openText,web_app:{url:appUrl}}],[{text:'📋 Правила',callback_data:'v12:rules'}]];if(access.admin)keyboard.push([{text:'👥 Сотрудники',callback_data:'v12:admin'}]);
...
async function handleCallback(cb:any,env:Env,origin:string){
  ...
  await tg(env.TELEGRAM_BOT_TOKEN,'answerCallbackQuery',{callback_query_id:cb.id});
  if(['v12:rules','rules','show_rules'].includes(data))return sendRulesV12(env,chatId);
  if(['v12:admin','admin','employees','staff'].includes(data)){if(isAdmin(env,id))return sendAdminV12(env,chatId);return send(env.TELEGRAM_BOT_TOKEN,chatId,'🔒 Управление сотрудниками доступно только администратору.');}
  if(!isAdmin(env,id))return;
  if(['v12:employees','employee_list','list_employees'].includes(data))return sendEmployeesV12(env,chatId);
